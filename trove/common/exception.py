...
class NoConfigParserFound(NotFound):
    message = _("No configuration parser found for service %(service_type)s")


class ConfigurationDatastoreNotMatchInstance(TroveError):
    message = _("Datastore on Configuration %(config_datastore)s does not "
                "match the Datastore on the instance %(instance_datastore)s.")
