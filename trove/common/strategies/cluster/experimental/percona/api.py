...
class PerconaAPIStrategy(base.BaseAPIStrategy):

    @property
    def cluster_class(self):
        return PerconaCluster
    ...
    def cluster_view_class(self):
        return PerconaClusterView

    @property
    def mgmt_cluster_view_class(self):
        return PerconaMgmtClusterView


class PerconaCluster(models.Cluster):

    @classmethod
    def create(cls, context, name, datastore, datastore_version, instances):
        LOG.debug("Initiating Percona cluster creation.")
        percona_conf = CONF.get(datastore_version.manager)
        num_instances = len(instances)

        # Matching number of instances with configured cluster_member_count
        if num_instances != percona_conf.cluster_member_count:
            raise exception.ClusterNumInstancesNotSupported(
                num_instances=percona_conf.cluster_member_count)
        ...
        volume_size = None
        if percona_conf.volume_support:
            ...
            ephemeral_support = percona_conf.device_path
        ...
        return PerconaCluster(context, db_info, datastore, datastore_version)


class PerconaClusterView(ClusterView):

    def build_instances(self):
        return self._build_instances(['member'], ['member'])


class PerconaMgmtClusterView(MgmtClusterView):
