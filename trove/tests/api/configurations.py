...
def _test_configuration_is_applied_to_instance(instance, configuration_id):
    ...
    for expected_key, expected_value in testconfig_info.values.iteritems():
        ...
        if isinstance(expected_value, bool):
            converted_value = 'ON' if expected_value else 'OFF'
            expected = (str(expected_key), converted_value)
        else:
...
class CreateConfigurations(object):
    ...
    def test_valid_configurations_create(self):
        ...
        values = '{"connect_timeout": 120, "local_infile": true, "collation_server": "latin1_swedish_ci"}'
    ...
    def test_appending_to_existing_configuration(self):
        raise SkipTest("Skipping because its not working with repose")
...
class ListConfigurations(object):
    ...
    def test_configurations_get(self):
        ...
        # check the items are the correct types with the rules
        for (item_key, item_val) in configuration_info.values.iteritems():
            print("item_key: %s" % item_key)
            print("item_val: %s" % item_val)
            param = instance_info.dbaas.configurations_parameters.get_parameter(item_key)
            if param.type == 'integer':
                assert_true(isinstance(item_val, int))
            if param.type == 'string':
                assert_true(isinstance(item_val, basestring))
            if param.type == 'boolean':
                assert_true(isinstance(item_val, bool))
