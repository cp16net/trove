    ...
    def update(self, req, id, body, tenant_id):
        ...
        if 'configuration_ref' in body["instance"]:
            ...
            if configuration_ref:
                conf = utils.get_id_from_href(
                    body["instance"]["configuration_ref"])
                LOG.debug("configuration set to : %s" % conf)
                config = models.Configuration.load(context, conf)
                if config.datastore_version_id != instance.datastore_version_id:
                    raise exception.DatastoreDoNotMatchException(
                        config_datastore=config.datastore,
                        instance_datastore=instance.datastore)
                config_items = models.Configuration.load_items(context, conf)
            ...
            if instance.configuration and instance.configuration.id:
