...
class InstanceController(wsgi.Controller):
    ...
    def update(self, req, id, body, tenant_id):
        ...
        if 'configuration_ref' in body["instance"]:
            ...
            # TODO(cp16net) we need to move this code to a separate API method
            # for unassigning a configuration

            # need to validate data here and pass to the instance model
            ...
            # configuration = cfg_model.load(context, config_id)

            # # get the default configuration values
            # default_template = template.SingleInstanceConfigTemplate('mysql', instance.flavor)
            # default_config = default_template.render_dict()
            # LOG.debug("default mysqld section: %s" % default_config)

            # # build a dict of values to unset
            # overrides = {}
            # for item in configuration.items:
            #     overrides[item.name] = default_config[item.name]
